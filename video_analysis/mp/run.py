...
cap = cv2.VideoCapture("/home/james/SMN_P6/video_analysis/gui/camil_follow.mp4")
ys_l_shoulder = []
ys_r_shoulder = []
i=0
with mp_pose.Pose(
  ...
  while cap.isOpened():
    ...
    if results.pose_landmarks!=None :
        ...
        #if x_axis is confound
        if landmarks[12][0] < landmarks[11][0] :
            print("turn")
            plt.scatter(i, np.mean([landmarks[11][1],landmarks[12][1]]), c='blue')
        ...
        i+=1
        #detect fall : idk how we can do to detect fall really.
    mp_drawing.draw_landmarks(
        image,
        results.pose_landmarks,
        mp_pose.POSE_CONNECTIONS,
    ...
    cv2.imshow(winname2, cv2.flip(image2, 1))
